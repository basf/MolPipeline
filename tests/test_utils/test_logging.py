...
class LoggingUtilsTest(unittest.TestCase):
    ...
    def test__print_elapsed_time(self) -> None:
        ...
        stream1 = io.StringIO()
        with (
            redirect_stdout(stream1),
            print_elapsed_time("source", message=None, use_logger=False),
        ):
            pass
        ...
        stream2 = io.StringIO()
        with (
            redirect_stdout(stream2),
            print_elapsed_time("source", message="my message", use_logger=False),
        ):
            pass
