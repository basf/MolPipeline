...
class BaseConformalTest(unittest.TestCase):
    """Base class for conformal prediction tests with shared setup."""
    ...
    def setUpClass(cls) -> None:  # pylint: disable=too-many-locals
        ...
        pipeline_clf = Pipeline([
            ...
            ("morgan", MolToMorganFP(radius=FP_RADIUS, n_bits=FP_SIZE, return_as="dense")),
            ("filter_reinserter", FilterReinserter.from_error_filter(error_filter_clf, fill_value=np.nan)),
        ])
        ...
        pipeline_reg = Pipeline([
            ...
            ("morgan", MolToMorganFP(radius=FP_RADIUS, n_bits=FP_SIZE, return_as="dense")),
            ("filter_reinserter", FilterReinserter.from_error_filter(error_filter_reg, fill_value=np.nan)),
        ])
        ...
        cls.smiles_reg = [smiles_list_reg.iloc[i] for i in range(len(smiles_list_reg)) if valid_mask_reg[i]]

    def _check_prediction_sets_content(self, prediction_sets: list[npt.NDArray[np.int_]], n_classes: int) -> None:
...
class TestConformalPredictor(BaseConformalTest):
    """Unit tests for ConformalPredictor wrapper."""
    ...
    def test_nonconformity_functions(self) -> None:
        ...
        sets_hinge_flat = [set(s.tolist()) for s in sets_hinge]
        sets_margin_flat = [set(s.tolist()) for s in sets_margin]
        self.assertTrue(any(h != m for h, m in zip(sets_hinge_flat, sets_margin_flat)))
    ...
    def test_mondrian_conformal_classification(self) -> None:
        ...
        # Test with custom MondrianCategorizer
...
class TestCrossConformalPredictor(BaseConformalTest):
    """Unit tests for CrossConformalPredictor wrapper."""
...
class TestConformalPipelineIntegration(BaseConformalTest):
    ...
    def test_pipeline_wrapped_by_conformal_classifier(self) -> None:
        ...
        conformal_pipeline = ConformalPredictor(base_pipeline, estimator_type="classifier")
    ...
    def test_pipeline_wrapped_by_conformal_regressor(self) -> None:
        ...
        conformal_pipeline = ConformalPredictor(base_pipeline, estimator_type="regressor")
    ...
    def test_conformal_in_pipeline_like_ml_experiments(self) -> None:
        ...
        conformal_pipeline = Pipeline([
            ("smi2mol", SmilesToMol()),
            ("mol2morgan", MolToMorganFP(radius=FP_RADIUS, n_bits=FP_SIZE, return_as="dense")),
            ("conformal_classifier", ConformalPredictor(
        ...
        conformal_pipeline.named_steps["conformal_classifier"].calibrate(x_calib, y_calib)
    ...
    def test_cross_conformal_in_pipeline_like_ml_experiments(self) -> None:
        ...
        cross_conformal_pipeline = Pipeline([
            ("smi2mol", SmilesToMol()),
            ("mol2morgan", MolToMorganFP(radius=FP_RADIUS, n_bits=FP_SIZE, return_as="dense")),
            ("cross_conformal_regressor", CrossConformalPredictor(
                RandomForestRegressor(n_estimators=5, random_state=42),
                estimator_type="regressor", n_folds=2, random_state=42
            )),
        ])
        train_smiles, test_smiles, train_values, test_values = train_test_split(
        ...
        intervals = cross_conformal_pipeline.named_steps["cross_conformal_regressor"].predict_int(
            Pipeline([
                ("smi2mol", SmilesToMol()),
                ("mol2morgan", MolToMorganFP(radius=FP_RADIUS, n_bits=FP_SIZE, return_as="dense")),
...
class TestConformalSerialization(BaseConformalTest):
    ...
    def test_json_serialization_cross_conformal_predictor(self) -> None:
        ...
        ccp = CrossConformalPredictor(
            estimator=clf, estimator_type="classifier", n_folds=2,
            random_state=42, confidence_level=0.8
        )
        ccp.fit(self.x_clf, self.y_clf)
    ...
    def test_json_serialization_cross_conformal_predictor_regression(self) -> None:
        ...
        ccp = CrossConformalPredictor(
            estimator=reg, estimator_type="regressor", n_folds=2,
            random_state=42, confidence_level=0.9
        )
    ...
    def test_json_serialization_pipeline_wrapped_by_conformal(self) -> None:
        ...
        conformal_wrapper = ConformalPredictor(base_pipeline, estimator_type="classifier")
    ...
    def test_joblib_serialization_conformal_predictor(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            model_path = os.path.join(temp_dir, "conformal_model.joblib")
            ...
            for orig_set, loaded_set in zip(original_sets, loaded_sets):
                ...
                if len(orig_set_items) > 0 or len(loaded_set_items) > 0:
                    ...
                    self.assertTrue(all(isinstance(item, (int, np.integer)) for item in all_items))
    ...
    def test_joblib_serialization_cross_conformal_predictor(self) -> None:
        ...
        reg = RandomForestRegressor(n_estimators=5, random_state=42)
        ccp = CrossConformalPredictor(reg, estimator_type="regressor", n_folds=2, random_state=42)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            model_path = os.path.join(temp_dir, "cross_conformal_model.joblib")
    ...
    def test_joblib_serialization_pipeline_wrapped_by_conformal(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            model_path = os.path.join(temp_dir, "wrapped_pipeline.joblib")
    ...
    def test_joblib_serialization_conformal_in_pipeline(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            model_path = os.path.join(temp_dir, "pipeline_with_conformal.joblib")
