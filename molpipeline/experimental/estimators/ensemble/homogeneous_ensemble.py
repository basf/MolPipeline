...
class HomogeneousEnsemble(abc.ABC, BaseEstimator, Generic[_ModelVar]):
    ...
    The design implemented here creates an ensemble of homogeneous models from the
    training set generated by the split method using the data provided for fitting the
    ensemble. The amount of models depends on the number of splits, generated by the
    split method. The data for each model must not necessarily differ, as the e.g. the
    DataRepetitionSplit yields the  full data set over and over.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        sampler: int | BaseCrossValidator, default=100
            ...
            If an int is passed, a BootstrapSplit with the respective number of
            splits is used. The BootstrapSplit, creates an independent bootstrap sample
            for each split, which is used to fit one member of the ensemble.
